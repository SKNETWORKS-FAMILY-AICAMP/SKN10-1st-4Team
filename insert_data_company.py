...
# CSV 파일 경로 설정
csv_file_path = os.path.join("data", "Company.csv")
...
try:
    with open(csv_file_path, mode="r", encoding="utf-8") as file:
        ...
        # INSERT 쿼리 생성
        query = f"INSERT INTO {table_name} (CompanyID, CompanyEngName, CompanyKorName) VALUES (%s, %s, %s)"
        
        # 데이터 삽입
        for row in csv_reader:
            mapped_row = (row[0], row[1], row[2])  # 순서 매핑
