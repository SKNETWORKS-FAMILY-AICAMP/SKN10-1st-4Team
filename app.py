...
with tab3:
    ...
    search_query = st.text_input("", key="gene_search_input", placeholder="검색어를 입력하세요...", label_visibility="collapsed", on_change=search)
    if search_query:
    ...
    with button_container:
        col1, col2, col3 = st.columns([1, 6, 1])
        with col1:
            if page > 1 and st.button("이전", key="gene_prev_page"):
                st.session_state.page = page - 1
        with col2:
            st.markdown(f"<div style='text-align: center;'>페이지 {page} / {total_pages}</div>", unsafe_allow_html=True)
        with col3:
            if page < total_pages and st.button("다음", key="gene_next_page"):
                st.session_state.page = page + 1